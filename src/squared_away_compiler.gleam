...
pub type CompilerOutput {
  CompilerOutput(bytecode: BitArray, errors_to_display: List(DisplayableError))
}

pub type DisplayableError {
  DisplayableError(title: String, location: #(#(Int, Int), #(Int, Int)))
}
...
fn displayable_from_scan_error(e: scanner.ScanError) -> DisplayableError {
  ...
  let title = case type_ {
    scanner.TrailingDecimal ->
      "Found trailing decimal after digits with no following digit."
...
fn displayable_from_parse_error(e: parser.ParseError) -> DisplayableError {
  ...
  let title = case type_ {
    ...
    parser.ExpectedOneOf(types:, got:) ->
      "Expected one of "
      <> string.inspect(types)
      <> " but got "
      <> string.inspect(got)
    parser.UnexpectedToken(t) ->
      "Was not expecting this token: " <> string.inspect(t)
  }

  DisplayableError(title:, location: #(span.start, span.end))
}
...
fn displayable_from_runtime_error(e: vm.RuntimeError) -> DisplayableError {
  ...
  DisplayableError(title: string.inspect(e), location: #(#(0, 0), #(0, 0)))
}

pub fn compile(src: String) -> CompilerOutput {
  case scanner.scan(src) {
    Error(e) ->
      CompilerOutput(bytecode: <<>>, errors_to_display: [
        displayable_from_scan_error(e),
      ])
    Ok(scanned) -> {
      ...
      CompilerOutput(
        bytecode:,
        errors_to_display: list.map(parse_errors, displayable_from_parse_error)
          |> list.append(list.map(type_errors, displayable_from_type_error)),
      )
    }
  }
}
